...
const List = () => {
    ...
    // Split items into purchased and unpurchased for non-owners
    const unpurchasedItems = items.filter(item => !item.purchases || item.purchases.length === 0);
    const purchasedItems = items.filter(item => item.purchases && item.purchases.length > 0);
        ...
        <div className="py-12">
            ...
            {(isOwner && !showPurchases) ? (
                ...
                <>
                    ...
                    {unpurchasedItems.length === 0 && purchasedItems.length > 0 && !isOwner && (
            ...
            {isOwner && (
                <div className="bg-gray-50 rounded-xl p-4 mt-4 mb-4">
                    <h3 className="text-sm font-semibold text-gray-700 mb-1">List Owner Controls</h3>
                    <p className="text-sm text-gray-500 mb-3">This is your list, so you get to manage it.</p>
                    <div className="flex flex-wrap gap-2">
                        <button
                            onClick={() => setShowEditModal(true)}
                            className="border border-pink-500 text-pink-500 hover:bg-pink-50 font-semibold py-2 px-4 rounded-lg transition-colors flex items-center gap-2 bg-white"
                        >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
